...
class oeGdprProductRatingDaoTest extends \PHPUnit_Framework_TestCase
{
    /**
     * @dataProvider invalidProductIdsProvider
     * @expectedException oeGdprInvalidObjectIdDaoException
     */
    public function testGetProductByIdWithInvalidId($invalidProductId)
    {
        $database = $this->getMockBuilder('Database')->getMock();
        $productRatingDao = new oeGdprProductRatingDao($database);
